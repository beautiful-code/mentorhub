...
.factory('MentoringTrackNewServices', ["$http", "ApiUrls", "Utils", function($http, ApiUrls, Utils) {
    return {
        ...
        putRequest: function(id, payload) {
            return $http.put('/requests/' + id, {
                mentoring_request: payload
            });
        }
    };
}])
...
.controller('MentoringTrackNew', ['$rootScope', '$scope', '$window', 'MentoringTrackNewServices', function($rootScope, $scope, $window, MentoringTrackNewServices) {
    ...
    var init = function() {
        ...
        $scope.current_user = MentoringTrackConfig.current_user;
    ...
    $scope.update_mentee_track = function() {
        MentoringTrackNewServices.getSectionsData({
                '{track_id}': $scope.selectTrack.id
            })
    ...
    $scope.addQuestion = function(index) {
        if (!($scope.selectTrack.sections[index].questions)) {
            $scope.selectTrack.sections[index].questions = [];
        }
        $scope.selectTrack.sections[index].questions.push({});
    };
    ...
    $scope.confirm_mentee_track = function(event) {
      var title = ($scope.current_user.id == $scope.selectTrack.mentee.id) ? "Would you like to learn " + $scope.selectTrack.name : "Would you like to mentor " + $scope.selectTrack.mentee.first_name + " on " + $scope.selectTrack.name;
        swal({
            title: title,
            ...
            if (isConfirm) {
                ...
                MentoringTrackNewServices.postTrackData($scope.selectTrack)
                    .success(function(response) {
                        ...
                        angular.forEach($scope.mentor_requests, function(mentor_request, index) {
                            if (($scope.track.mentee_id == mentor_request.mentee_id) &&
                                ...
                                MentoringTrackNewServices.putRequest(mentor_request.id, {
                                        state: 'accepted'
                                    })
                                    .success(function(response) {
                    ...
                    .error(function(response) {
                        if (response.msg == "track present") {
                            $(event.target).button('reset');
                            var errorMsg = ($scope.current_user.id == $scope.selectTrack.mentee.id) ? "You are already learning " + $scope.selectTrack.name : "You are already mentoring " + $scope.selectTrack.mentee.first_name + " on " + $scope.selectTrack.name;
                            swal(errorMsg);
                        }
                    });
            }
